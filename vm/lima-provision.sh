...
# Add PAM group rules to /etc/security/group.conf
# This makes pam_group.so grant docker, kvm, and podman groups to all SSH sessions
cat >> /etc/security/group.conf <<'GROUPCONF'

# Auto-generated by lima-provision.sh
# Grant container and VM groups to all users via SSH
sshd;*;*;Al0000-2400;docker,kvm,podman
GROUPCONF

if ! grep -q "pam_group.so" /etc/pam.d/sshd 2>/dev/null; then
    # Add pam_group.so to SSH PAM configuration (AUTH section - required by pam_group)
    # pam_group(8) man page states: "Only the auth module type is provided"
    # Must be in auth section, not session section, to function properly
